  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-surface-app/40 border border-border-subtle p-3 rounded-xl select-none">
            ...
            <button
              ...
              className={`px-3 py-1 rounded-md text-[10px] font-semibold transition-colors ${
                period === t.key && !customStart
                  ? 'bg-surface-panel text-accent border border-border-subtle/50'
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="bg-surface-app border border-border-subtle rounded px-2 py-1 text-[10px] text-white"
          />
        ...
        <div className="flex items-center gap-2 ml-auto">
          <button
            onClick={() => refetch()}
            className="p-2 border border-border-subtle hover:border-white rounded-lg text-text-secondary hover:text-white transition-colors"
          ...
          <button
            ...
            className="p-2 border border-border-subtle hover:border-white rounded-lg text-text-secondary hover:text-white transition-colors disabled:opacity-30"
            title="Export CSV"
          >
            <Download size={12} />
          </button>
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-surface-panel/40 border border-border-subtle rounded-lg p-2.5">
          <p className="text-[10px] text-text-secondary font-medium">Opening Balance</p>
          <p className="text-xs font-mono font-bold text-white mt-0.5">{formatPaise(openingBalance)}</p>
        </div>
        <div className="bg-surface-panel/40 border border-border-subtle rounded-lg p-2.5">
          <p className="text-[10px] text-text-secondary font-medium font-mono text-red-400">Total Debit (DR)</p>
          <p className="text-xs font-mono font-bold text-red-400 mt-0.5">+{formatPaise(totalDebit)}</p>
        </div>
        <div className="bg-surface-panel/40 border border-border-subtle rounded-lg p-2.5">
          <p className="text-[10px] text-text-secondary font-medium font-mono text-brand-500">Total Credit (CR)</p>
          <p className="text-xs font-mono font-bold text-brand-500 mt-0.5">-{formatPaise(totalCredit)}</p>
        </div>
        <div className="bg-surface-panel/40 border border-border-subtle rounded-lg p-2.5">
          <p className="text-[10px] text-text-secondary font-medium">Closing Balance</p>
          <p className="text-xs font-mono font-bold text-white mt-0.5">{formatPaise(closingBalance)}</p>
        </div>
      </div>

      {/* Ledger Table */}
      <div className="bg-surface-panel border border-border-subtle rounded-xl overflow-hidden print:border-0 print:bg-white print:text-black">
        <div className="overflow-x-auto">
          <table className="w-full text-xs text-left">
            <thead>
              <tr className="border-b border-border-subtle bg-surface-app print:bg-slate-200 print:text-black">
                ...
                <th className="px-3 py-2.5 text-right text-red-400 font-semibold print:text-red-600">Debit (DR)</th>
                <th className="px-3 py-2.5 text-right text-brand-500 font-semibold print:text-emerald-700">Credit (CR)</th>
            ...
            <tbody className="divide-y divide-border-subtle print:divide-slate-200">
              {isLoading ? (
                Array.from({ length: 4 }).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    {Array.from({ length: 7 }).map((__, j) => (
                      <td key={j} className="px-4 py-3"><div className="h-2.5 bg-surface-app rounded" /></td>
                    ))}
                  </tr>
                ))
              ) : entries.length === 0 ? (
                ...
                entries.map((r: LedgerEntry) => (
                  <tr key={r.id} className="hover:bg-surface-app/20 transition-colors print:hover:bg-transparent">
                    <td className="px-4 py-2.5 font-mono text-white print:text-black">{formatDate(r.entry_date)}</td>
                    <td className="px-3 py-2.5">
                      <span className="font-semibold text-[10px] uppercase text-text-secondary print:text-black">
                        {getLedgerRefLabel(r.ref_type as any)}
                      </span>
                    </td>
                    <td className="px-3 py-2.5 font-mono text-white print:text-black">{r.invoice_number || '—'}</td>
                    <td className="px-4 py-2.5 text-white/90 print:text-black">{r.description}</td>
                    <td className="px-3 py-2.5 text-right font-mono font-semibold text-red-400 print:text-red-600">
                      {r.debit_paise > 0 ? `+${formatPaise(r.debit_paise)}` : '—'}
                    </td>
                    <td className="px-3 py-2.5 text-right font-mono font-semibold text-brand-500 print:text-emerald-700">
